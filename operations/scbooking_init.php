   ...
   $wpdb->query($sql_scpm);	 
	...
	if(!$curr_count[0]->rowcount){
	 ...
	 if(empty($scbooking_calendar_page)){
	 	$scbooking_calendar_page_id = sc_programmatically_create_page('Booking Calendar','party-hall-booking-calendar','[sc_sccalendar]','page');
	 }
	 if(empty($scrooms_page)){
	 	$sc_rooms_id = sc_programmatically_create_page('Party Hall','sc-rooms','[sc_roomsgallery]','page');
	 }
	 if(empty($roomdetails_page)){
	 	$room_details_id = sc_programmatically_create_page('Party Hall Details','room-details','[sc_scbooking_roomdetails]','page');
	 }	 
}