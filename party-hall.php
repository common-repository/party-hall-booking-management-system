<?php
/*
Plugin Name: WP Party Hall Booking Manager
Plugin URI: http://www.codeinterest.com/
Description: WP Party Hall Booking Management System
Version: 1.2
Author: SolverCircle
Author URI: http://www.solvercircle.com
Text Domain: sc-scbooking
*/
//free Party Hall Booking Manager plugin
...
$scrooms_page =  get_page_by_title('Party Hall');
$roomdetails_page =  get_page_by_title('Party Hall Details');
...
function sc_create_custom_post_type() {
	register_post_type( 'sc_custom_booking',
		array(
			'labels' => array(
				'name' => __('Rooms','sc-scbooking'),
				'singular_name' => __('Room','sc-scbooking'),
				'menu_name'=>__('Party Hall Booking Manager','sc-scbooking'),
				'all_items'=>__('Party Hall','sc-scbooking'),
				'add_new_item'=>__('Add New Party Hall','sc-scbooking'),
				'add_new'=> __('Add New Party Hall','sc-scbooking'),
				'not_found'=>__('No Party Hall Found.','sc-scbooking'),
				'search_items'=>__('Search Hall','sc-scbooking'),
...
add_meta_box(
		...
		''.__('Party Hall Attribute Settings','sc-scbooking').'', // Meta Box Title
...
<table>
  <tbody>
    <tr>
      <th scope="row"><?php _e("Total Hall:","sc-scbooking"); ?></th>
...
function sc_save_room_metabox($post_id){
	...
	if( $_POST ) :
		...
		if(isset($_POST['roommetabox_noofbed'])){	
			...
			$safe_zipcode = intval($room_noofbed);
			if ( ! $room_noofbed ) {
			  $room_noofbed = 1;
			}
		}
    ...
    $room_currency = "";
		...
		if(isset($_POST['roommetabox_Description'])){
			$room_description = sanitize_text_field( $_POST['roommetabox_Description'] );
		}
		$room_image = "";
...
function sc_cssfix_front(){
	add_submenu_page( 'edit.php?post_type=sc_custom_booking', 'FrontEnd CSS Fix', ''.__("FrontEnd CSS Fix","sc-scbooking").'', 'manage_options', 'css-fix-menu', 'sc_cssfix_front_setting' );
}
...
function sc_cssfix_front_setting(){
...
add_action('admin_menu','sc_cssfix_front');
          ...
          $msg .= '<table class="wp-list-table widefat fixed bookmarks" cellspacing="0">
                      <thead>
                        <tr>
                          ...
                          <th>'.__("Booking date","sc-scbooking").'</th>                          
                  ...
                  foreach($result_page_data as $booking){
                    ...
                    $msg .= '<tr class="alternate">
                                ...
                                <td colspan="2">
                            ';
                    $msg .= '<a id="delete_booking" href="#" >'.__("delete","sc-scbooking").'</a>
...
function sc_save_paymentmethod(){
  if ( count($_POST) > 0 ){
      ...
      $paymentmethod_name = sanitize_title($_REQUEST['paymentmethod']);
      $values = array(
      ...
      if($hdnpaymentmethodid == "" || $hdnpaymentmethodid == NULL){
        ...
        $wpdb->update(
            $table_prefix.'sc_scbooking_paymentmethods',
            $values,
            array('id' => $hdnpaymentmethodid)
         );
      ...
      $msg .= '<table class="wp-list-table widefat fixed bookmarks" cellspacing="0">
                  <thead>
                    <tr>
                      ...
                      <th>'.__("Hall","sc-scbooking").'</th>
                      <th>'.__("Date","sc-scbooking").'</th>                      
                      <th>'.__("Email","sc-scbooking").'</th>
                      <th>'.__("Phone","sc-scbooking").'</th>
...
//=====fullcalendar script===================
    ...
    wp_enqueue_script( 'jquery.multiple.select' );
    wp_enqueue_script( 'jquery.bt.min' );
		//wp_enqueue_script( 'jqueryui_min' );
