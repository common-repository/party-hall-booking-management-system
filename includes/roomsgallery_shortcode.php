...
function sc_roomsgallery_shortcode($atts){
  ...
  if(!get_option('scpd_base_currency')){
    ...
    $room_country_currency=scpd_currency_code(get_option('scpd_base_currency'));
		...
		<div><a href="'.get_option('siteurl').'/?page_id='.ROOMDETAILS_PAGEID.'&roomid='.$postid.'">'.sprintf(__("%s","sc-scbooking"),sanitize_title($room->post_title)).'</a></div><div>'.$room_country_currency.''.sprintf(__("%s","sc-scbooking"),$room_price[0]->meta_value).''.__("/Day","sc-scbooking").'</div>
		</div>';
	}
	$output .= '</div>';
	return $output;
